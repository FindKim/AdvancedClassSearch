					...
					for pcourse in schedule:
						for day1 in tempcourse.days:
							for day2 in pcourse.days:
								if(day1 == day2): #or ((day1 != day2) and exday == 1)):
									works = 1
									pcourse.starttime = float(pcourse.starttime)
									pcourse.endtime = float(pcourse.endtime)
									tempcourse.starttime = float(pcourse.starttime)
									pcourse.endtime = float(pcourse.endtime)
									if ((pcourse.starttime <= tempcourse.starttime <= pcourse.endtime)):
										works = 0
										break 
									elif ((pcourse.starttime <= tempcourse.endtime <= pcourse.endtime)):
										works = 0 
										break
									elif ((tempcourse.starttime <= pcourse.starttime <= tempcourse.endtime)):
										works = 0
										break
									elif ((tempcourse.starttime <= pcourse.endtime <= tempcourse.endtime)):
										works = 0
										break
									else:
										#exday=1
										print "day 1", day1
										#print isinstance(tempcourse.starttime, str)
                                                                                print "StartTime1", tempcourse.starttime
										#iftrue = ((tempcourse.starttime <= pcourse.starttime) and (tempcourse.endtime >= pcourse.endtime))
                                                                                #iftrue2 =((tempcourse.starttime >= pcourse.starttime) and (tempcourse.endtime <= pcourse.endtime))
